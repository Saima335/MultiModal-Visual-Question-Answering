...
def are_keys_correct(pred):
    for answer in pred.values():
        if not(answer.upper() == 'A' or answer.upper() == 'B' or answer.upper() == 'C' or answer.upper() == 'D' or answer.upper()== 'E'):
            # raise Exception('Unsupported answer_key: {}. Cannot score predictions.'.format(answer))4
            continue
...
def read_data(path, lang=None):
    ...
    for el in jsn_data:
        if ID_FIELD_NAME not in jsn_data[el].keys() or ANSWER_FIELD_NAME not in jsn_data[el].keys():
            raise Exception('Prediction file format is invalid. Each object must contain the following fields: [id, answer_key]')
        if lang:
            if lang == el['language']:
                data[jsn_data[el][ID_FIELD_NAME]] = jsn_data[el][ANSWER_FIELD_NAME]
        else:
            data[jsn_data[el][ID_FIELD_NAME]] = jsn_data[el][ANSWER_FIELD_NAME]
    return data
